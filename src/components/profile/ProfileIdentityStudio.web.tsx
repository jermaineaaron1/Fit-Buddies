...
export function ProfileIdentityStudio() {
  ...
  async function upload(file?: File) {
    ...
    if (!file.type.startsWith('image/')) return Alert.alert('Choose an image', 'Select a JPG, PNG, or WebP file.')
    if (file.size > 5 * 1024 * 1024) return Alert.alert('Image too large', 'Choose an image smaller than 5 MB.')
    setUploading(true)
    ...
    if (uploadError) { setUploading(false); return Alert.alert('Upload failed', uploadError.message) }
    const { data: publicData } = supabase.storage.from('avatars').getPublicUrl(path)
    const { data, error } = await supabase.from('profiles').update({ avatar_url: publicData.publicUrl, avatar_source: 'photo' }).eq('id', profile.id).select('*').single()
    setUploading(false)
    if (error || !data) return Alert.alert('Could not save photo', error?.message ?? 'Please try again.')
    setProfile(data)
  }

  async function chooseSource(source: 'photo' | 'ai') {
    if (!profile) return
    if (source === 'ai' && !profile.ai_avatar_url) return Alert.alert('AI fighter avatar', 'Upload a source photo first. Avatar generation will use the server-side generator so your API credentials never appear in the app.')
    const { data, error } = await supabase.from('profiles').update({ avatar_source: source }).eq('id', profile.id).select('*').single()
    if (error || !data) return Alert.alert('Could not switch identity', error?.message ?? 'Please try again.')
    setProfile(data)
  }

  return <Card style={styles.card}>
    <View style={styles.header}><View style={styles.titleRow}><Ionicons name="camera-outline" size={20} color={colors.primary} /><View><Text style={styles.title}>Broadcast Identity</Text><Text style={styles.subtitle}>Visible to your circle and used on versus screens</Text></View></View><View style={styles.live}><Text style={styles.liveText}>PUBLIC IN CIRCLE</Text></View></View>
...
const styles = StyleSheet.create({
  card: { gap: 16 }, header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12 }, titleRow: { flexDirection: 'row', alignItems: 'center', gap: 9 }, title: { color: colors.text, fontFamily: type.display, fontSize: 20, fontWeight: '900', textTransform: 'uppercase' }, subtitle: { color: colors.textMuted, fontSize: 11, marginTop: 2 }, live: { paddingHorizontal: 9, paddingVertical: 5, borderRadius: radius.full, backgroundColor: colors.primary }, liveText: { color: '#fff', fontSize: 9, fontWeight: '900', letterSpacing: .7 },
